...
import {
  ...
  createAuthChallenge,
  normalizeEmail,
} from '../services/webAuthChallenge';
import { sendVerificationEmail } from '../services/emailTemplate';
import {
  PASSWORD_MAX_LENGTH,
  PASSWORD_MIN_LENGTH,
} from '../services/webAuthPassword';
import { EmailRegistrationSchema } from '../services/webAuthRegistration';
...
const EMAIL_REGISTRATION_PURPOSE = 'email-registration';
const PHONE_REGISTRATION_PURPOSE = 'phone-registration';
...
const PhoneVerifySchema = PhoneSchema.extend({
  code: z.string().regex(/^\d{6}$/),
});
const EmailVerifySchema = EmailSchema.extend({
  code: z.string().regex(/^\d{6}$/),
});
...
function reservePasswordLoginAttempt(
  email: string,
  ip: string
): Promise<boolean> {
  const identityHash = createHash('sha256')
    .update(`${email}:${ip}`)
    .digest('hex');
  return reserveExpiringKey(
    `web:auth:password:rate:${identityHash}`,
    PASSWORD_LOGIN_RATE_SECONDS
  );
}

function getPlayerComponent(): PlayerComponent | undefined {
  return ComponentManager.instance.getComponentByKey<PlayerComponent>(
    'PlayerComponent'
  );
}
...
async function sendAliyunSms(phone: string, code: string): Promise<void> {
  ...
  if (response.body?.code !== 'OK')
    throw new Error(response.body?.message ?? 'Aliyun SMS rejected request');
}
...
function buildWechatStartUrl(state: string): string {
  const params = new URLSearchParams({
    appid: getRequiredEnv('WECHAT_WEB_APP_ID'),
    redirect_uri: getRequiredEnv('WECHAT_WEB_REDIRECT_URI'),
    response_type: 'code',
    scope: 'snsapi_login',
    state,
  });
...
async function sendPhoneCode(
  ...
  try {
    const result = await createAuthChallenge(
      ...
      parsed.data.phone,
      getClientIp(req),
      (code) => sendAliyunSms(parsed.data.phone, code)
    );
    if (!result.ok)
      return sendErr(res, 'Please wait before requesting another code', 429);
...
router.post('/sms/send', (req, res) =>
  sendPhoneCode(req, res, PHONE_LOGIN_PURPOSE)
);
router.post('/email/register/send', (req, res) =>
  sendEmailCode(req, res, EMAIL_REGISTRATION_PURPOSE)
);
router.post('/phone/register/send', (req, res) =>
  sendPhoneCode(req, res, PHONE_REGISTRATION_PURPOSE)
);
...
router.post('/sms/verify', async (req: Request, res: Response) => {
  ...
  if (!verified)
    return sendErr(res, 'Verification code is invalid or expired', 401);
  const playerComp = getPlayerOrRespond(res);
...
router.post('/email/password/login', async (req: Request, res: Response) => {
  ...
  if (!parsed.success)
    return sendErr(res, 'Invalid email address or password', 400);
  const email = normalizeEmail(parsed.data.email);
  const rateAllowed = await reservePasswordLoginAttempt(
    email,
    getClientIp(req)
  );
  ...
  const playerComp = getPlayerOrRespond(res);
  if (!playerComp) return;
  const login = await playerComp.loginByEmailPassword(
    email,
    parsed.data.password
  );
  if (!login.ok)
    return sendErr(res, 'Email address or password is incorrect', 401);
  await sendLoginSuccess(res, login.data.userId);
});
...
router.post('/email/password/reset', async (req: Request, res: Response) => {
  ...
  const playerComp = getPlayerOrRespond(res);
  if (!playerComp) return;
  const updated = await playerComp.updateEmailPassword(
    email,
    parsed.data.password
  );
...
router.post('/email/register', async (req: Request, res: Response) => {
  const parsed = EmailRegistrationSchema.safeParse(req.body);
  if (!parsed.success) return sendErr(res, 'Invalid registration details', 400);
  const email = normalizeEmail(parsed.data.email);
  const emailVerified = await consumeAuthChallenge(
    'email',
    EMAIL_REGISTRATION_PURPOSE,
    email,
    parsed.data.emailCode
  );
  if (!emailVerified)
    return sendErr(res, 'Email verification code is invalid or expired', 401);
  if (parsed.data.phone && parsed.data.phoneCode) {
    const phoneVerified = await consumeAuthChallenge(
      'sms',
      PHONE_REGISTRATION_PURPOSE,
      parsed.data.phone,
      parsed.data.phoneCode
    );
    if (!phoneVerified)
      return sendErr(res, 'Phone verification code is invalid or expired', 401);
  }
  const playerComp = getPlayerOrRespond(res);
  if (!playerComp) return;
  const registered = await playerComp.registerWebUser(
    email,
    parsed.data.password,
    parsed.data.phone
  );
  if (!registered.ok) {
    const statusCode = registered.error === 'CredentialBound' ? 409 : 500;
    const message =
      statusCode === 409
        ? 'Account already exists or a credential is in use'
        : 'Unable to create account';
    return sendErr(res, message, statusCode);
  }
  await sendLoginSuccess(res, registered.data.userId);
});

async function startWechat(
  req: MiniappRequest | Request,
  res: Response,
  stateData: WechatState
): Promise<void> {
  try {
    ...
    logger.error('web WeChat start failed', {
...
router.get('/wechat/start', (req, res) =>
  startWechat(req, res, { purpose: 'login' })
);
router.post(
  '/wechat/bind/start',
  authMiddleware,
  async (req: MiniappRequest, res) => {
    ...
    try {
      ...
      await saveExpiringValue(
        ...
        JSON.stringify({
          purpose: 'bind',
          userId: req.userId,
        } satisfies WechatState),
        WECHAT_STATE_TTL_SECONDS
      );
      sendSucc(res, { url: buildWechatStartUrl(state) });
    } catch (error) {
      logger.error('web WeChat bind start failed', {
        error: (error as Error).message,
      });
...
// This callback coordinates the provider exchange, identity binding, and session handoff.
// eslint-disable-next-line max-lines-per-function
router.get('/wechat/callback', async (req: Request, res: Response) => {
  const code = typeof req.query.code === 'string' ? req.query.code : '';
  const state = typeof req.query.state === 'string' ? req.query.state : '';
  const rawState = state
    ? await consumeExpiringValue(`web:wx:state:${state}`)
    : null;
  if (!code || !rawState)
    return sendErr(res, 'Invalid WeChat authorization response', 400);
  try {
    ...
    if (storedState.purpose === 'bind' && storedState.userId) {
      const bound = await playerComp.bindWechatIdentity(
        storedState.userId,
        identity.openid,
        identity.unionid
      );
      if (!bound.ok)
        return sendErr(res, 'This WeChat account is already bound', 409);
      return res.redirect(
        `${getRequiredEnv('WEB_AUTH_RETURN_URL')}?wechat_bound=1`
      );
    }
    const login = await playerComp.loginByWechatIdentity(
      identity.openid,
      identity.unionid
    );
    if (!login.ok) return sendErr(res, 'Unable to sign in', 500);
    ...
    logger.error('web WeChat callback failed', {
      error: (error as Error).message,
    });
...
router.post('/wechat/exchange', async (req: Request, res: Response) => {
  ...
  if (!/^[a-f0-9]{48}$/.test(ticket))
    return sendErr(res, 'Invalid login ticket', 400);
...
router.get(
  ...
  async (req: MiniappRequest, res: Response) => {
    const playerComp = getPlayerOrRespond(res);
    if (!playerComp || !req.userId) return;
...
router.post('/profile/phone/send', authMiddleware, (req, res) =>
  sendPhoneCode(req, res, PHONE_BIND_PURPOSE)
);
router.post('/profile/email/send', authMiddleware, (req, res) =>
  sendEmailCode(req, res, EMAIL_BIND_PURPOSE)
);
...
router.post(
  ...
  async (req: MiniappRequest, res: Response) => {
    ...
    if (!parsed.success || !req.userId)
      return sendErr(res, 'Invalid phone number or code', 400);
    const verified = await consumeAuthChallenge(
      'sms',
      PHONE_BIND_PURPOSE,
      parsed.data.phone,
    ...
    if (!bound.ok)
      return sendErr(res, 'This phone number is already bound', 409);
...
router.post(
  ...
  async (req: MiniappRequest, res: Response) => {
    ...
    if (!parsed.success || !req.userId)
      return sendErr(res, 'Invalid email address or code', 400);
    const email = normalizeEmail(parsed.data.email);
    const verified = await consumeAuthChallenge(
      'email',
      EMAIL_BIND_PURPOSE,
    ...
    if (!bound.ok)
      return sendErr(res, 'This email address is already bound', 409);
    sendSucc(res, { bound: true });
  }
);
