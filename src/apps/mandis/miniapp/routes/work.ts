...
async function processBase64Image(
  ...
  if (buffer.length === 0) return { error: '图片 data 为空' };
  const imgCheck = await checkImage(buffer, 'image/png');
  if (!imgCheck.safe) return { error: '图片疑似违规，请更换后重试' };
  ...
  const key = `${worksObjectPrefix}/${authorId}/${workId}-${index}${safeExt}`;
...
async function processPublishImages(
  ...
  for (let i = 0; i < rawImages.length; i++) {
    ...
    if (data) {
      const result = await processBase64Image(data, name, type, authorId, workId, i);
      if ('error' in result) { sendErr(res, result.error, 400); return null; }
      images.push(result);
      continue;
    }
    if (url && isTempOrUnfetchableUrl(url)) {
      sendErr(res, '请先通过 POST /api/upload 上传图片，将返回的 url 填入发布内容；或使用 data 字段提交 base64 图片', 400);
      return null;
    }
...
function resolveWorkImages(work: IWork): { url: string; name?: string; type?: string }[] {
  ...
  return (work.images as { url?: string; name?: string; type?: string }[]).map((img) => {
...
router.post('/publish', async (req: MiniappRequest, res: Response) => {
  ...
  logRequest('work.ts:publish:entry', 'publish request', {
    req, params: req.params ?? {}, requestBody: payload,
...
router.post('/publishDraft', async (req: MiniappRequest, res: Response) => {
  ...
  try {
    ...
    if (work.status === 'published') { sendSucc(res, { workId, message: 'Already published' }); return; }
