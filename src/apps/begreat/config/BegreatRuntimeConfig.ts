...
function load(): RuntimeConfig {
  ...
  return {
    ...
    dev_openids:     Array.isArray(data.dev_openids)           ? data.dev_openids     : DEFAULTS.dev_openids,
  };
}
...
export function initRuntimeConfig(): void {
  _current = load();
  logger.info(`[BegreatRuntimeConfig] 已加载: price_fen=${_current.price_fen} payment_enabled=${_current.payment_enabled} dev_openids=[${_current.dev_openids.join(',')}]`);
}
...
export function reloadRuntimeConfig(): RuntimeConfig {
  ...
  logger.info(
    ...
    `dev_openids: [${prev.dev_openids.join(',')}] → [${_current.dev_openids.join(',')}]`
  );
  return { ..._current };
}
