...
function buildFreeAiRisk(
  ...
  let bandKey: 'low' | 'medium' | 'high';
  if (aiRisk <= bands.low.max) {
    bandKey = 'low';
  } else if (aiRisk <= bands.medium.max) {
    bandKey = 'medium';
  } else {
    bandKey = 'high';
  }
 ...
 */
type FacetInsight = { facet: string; label: string; score: number; insight: string };

function buildFacetInsights(facetMeans: Record<string, number>): FacetInsight[] {
