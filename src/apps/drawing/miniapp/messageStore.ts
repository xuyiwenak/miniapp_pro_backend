...
/** 初始化演示用的默认会话数据 */
function buildDefaultSessions(): SessionItem[] {
  return [
    {
      userId: 1, name: 'Sean', avatar: '/static/chat/avatar-Sean.png',
      messages: [
        ...
        { messageId: 4, from: 1, content: '有个问题想咨询一下，关于TDesign组件库如何更好地使用', time: Date.now() - 3600000, read: false },
      ],
    },
    { userId: 2, name: 'Mollymolly', avatar: '/static/chat/avatar-Mollymolly.png',
      messages: [{ messageId: 5, from: 1, content: '好久不见，最近咋样？', time: 1692100800000, read: true }] },
    { userId: 3, name: 'Andrew', avatar: '/static/chat/avatar-Andrew.png',
      messages: [{ messageId: 6, from: 0, content: '现在没空，晚点再联系你哈', time: 1690084800000, read: true }] },
    { userId: 4, name: 'Kingdom', avatar: '/static/chat/avatar-Kingdom.png',
      messages: [{ messageId: 7, from: 1, content: '真的吗？', time: 1656880200000, read: true }] },
    { userId: 5, name: 'Paige', avatar: '/static/chat/avatar-Paige.png',
      messages: [{ messageId: 8, from: 1, content: '此次要评审的首页和专区页改版的交互方案', time: 1652963880000, read: true }] },
  ];
}
...
function addMessageToList(
  ...
  if (!session) {
    session = { userId: targetUserId, name: `User ${targetUserId}`, avatar: '/static/chat/avatar-default.png', messages: [] };
    list.unshift(session);
  }
  const msg: ChatMessage = { messageId: nextMessageId++, from, content, time: Date.now(), read: from === 0 };
...
function getStore(): MessageStoreAPI {
  return {
    getUnreadCount(ownerUserId: string): number {
      let n = 0;
      for (const s of ensureSessions(ownerUserId)) n += s.messages.filter((m) => !m.read).length;
      return n;
    },
    getSessionList(ownerUserId: string): SessionItem[] {
    ...
    markRead(ownerUserId: string, targetUserId: string): void {
      const session = ensureSessions(ownerUserId).find((s) => String(s.userId) === String(targetUserId));
      if (session) session.messages.forEach((m) => (m.read = true));
    },
    addMessage(ownerUserId: string, targetUserId: string | number, from: 0 | 1, content: string): ChatMessage {
      return addMessageToList(ensureSessions(ownerUserId), targetUserId, from, content);
    },
    addMessageFromTo(senderUserId: string, peerUserId: string, content: string) {
      const forSender = addMessageToList(ensureSessions(senderUserId), peerUserId, 0, content);
      const forPeer = addMessageToList(ensureSessions(peerUserId), senderUserId, 1, content);
