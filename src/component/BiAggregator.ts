...
export class BiAggregator implements IBaseComponent {
  ...
  private async aggregateHourlyForType(
    ...
    const results = await this.biEvent.aggregate<Record<string, unknown>>(
      buildHourlyPipeline(appName, eventType, periodStart, periodEnd),
    );
    if (!results.length || (results[0].totalEvents as number) === 0) return;

    const r = results[0];
    const metrics = buildHourlyMetrics(r, { periodStart, periodEnd, appName, eventType });
  ...
  private async aggregateDailyForType(appName: AppName, eventType: EventType, periodStart: Date, periodEnd: Date) {
    const hMetrics = await this.biHourly
      .find({ appName, eventType, periodStart: { $gte: periodStart, $lt: periodEnd } })
      .lean();
    if (!hMetrics.length) return;
    const totalEvents = hMetrics.reduce((s, m) => s + m.totalEvents, 0);
    const wAvg = totalEvents
      ? hMetrics.reduce((s, m) => s + m.avgDurationMs * m.totalEvents, 0) / totalEvents
      : 0;
    const daily: Partial<IBiMetricsHourly> = {
      ...
      avgDurationMs: wAvg,
      p50DurationMs: max(hMetrics.map(m => m.p50DurationMs)),
      p95DurationMs: max(hMetrics.map(m => m.p95DurationMs)),
      p99DurationMs: max(hMetrics.map(m => m.p99DurationMs)),
      maxDurationMs: max(hMetrics.map(m => m.maxDurationMs)),
      createdAt: new Date(),
      updatedAt: new Date(),
    };
...
type HourlyContext = Pick<IBiMetricsHourly, 'periodStart' | 'periodEnd' | 'appName' | 'eventType'>;

function buildHourlyPipeline(appName: AppName, eventType: EventType, periodStart: Date, periodEnd: Date) {
  return [
    { $match: { appName, eventType, timestamp: { $gte: periodStart, $lt: periodEnd } } },
    {
      $group: {
        ...
        qwenTokens: {
          $push: { $cond: [{ $eq: ['$eventType', 'qwen_analyze'] }, '$data.totalTokens', '$$REMOVE'] },
        },
        ...
        apiEndpoints: {
          $push: { $cond: [{ $eq: ['$eventType', 'api_request'] }, '$data.endpoint', '$$REMOVE'] },
        },
        apiStatusCodes: {
...
function buildHourlyMetrics(r: Record<string, unknown>, ctx: HourlyContext): Partial<IBiMetricsHourly> {
...
function uploadSub(r: Record<string, unknown>): NonNullable<IBiMetricsHourly['upload']> {
  ...
  return {
    totalBytes: sum(b),
    avgBytes: b.length ? sum(b) / b.length : 0,
    totalImages: b.length,
    contentTypes: count(r.uploadContentTypes as string[]),
  };
}
function qwenSub(r: Record<string, unknown>): NonNullable<IBiMetricsHourly['qwen']> {
  ...
  return {
    totalTokens: sum(t),
    totalCost: sum(c),
    avgTokensPerRequest: t.length ? sum(t) / t.length : 0,
    models: count(r.qwenModels as string[]),
  };
}
function apiSub(r: Record<string, unknown>): NonNullable<IBiMetricsHourly['api']> {
  ...
  return {
    endpoints: count(r.apiEndpoints as string[]),
    statusCodes: count(r.apiStatusCodes as string[]),
    totalRequestBytes: sum(rb),
    totalResponseBytes: sum(rs),
  };
}
...
function mergeMaps(maps: Array<Record<string, number>>): Record<string, number> {
  return maps.reduce((acc, m) => {
    for (const [k, c] of Object.entries(m)) acc[k] = (acc[k] || 0) + c;
    return acc;
  }, {} as Record<string, number>);
}
...
function mergeUpload(hm: IBiMetricsHourly[]): NonNullable<IBiMetricsHourly['upload']> {
  ...
  return {
    ...
    avgBytes: ti ? tb / ti : 0,
    totalImages: ti,
    contentTypes: mergeMaps(hm.map(m => m.upload?.contentTypes ?? {})),
  };
}
function mergeQwen(hm: IBiMetricsHourly[], totalEvents: number): NonNullable<IBiMetricsHourly['qwen']> {
  ...
  return {
    totalTokens: tt,
    totalCost: hm.reduce((s, m) => s + (m.qwen?.totalCost ?? 0), 0),
    avgTokensPerRequest: totalEvents ? tt / totalEvents : 0,
    models: mergeMaps(hm.map(m => m.qwen?.models ?? {})),
  };
}
function mergeApi(hm: IBiMetricsHourly[]): NonNullable<IBiMetricsHourly['api']> {
  return {
    endpoints: mergeMaps(hm.map(m => m.api?.endpoints ?? {})),
    statusCodes: mergeMaps(hm.map(m => m.api?.statusCodes ?? {})),
    totalRequestBytes: hm.reduce((s, m) => s + (m.api?.totalRequestBytes ?? 0), 0),
    totalResponseBytes: hm.reduce((s, m) => s + (m.api?.totalResponseBytes ?? 0), 0),
  };
}
