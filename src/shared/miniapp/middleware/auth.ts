...
export async function authMiddleware(
  ...
  if (!auth || !auth.startsWith("Bearer ")) {
    gameLogger.info(`[auth] 401 Unauthorized: ${req.method} ${req.path ?? (req as any).url} (no Bearer token)`);
  ...
  if (!userId) {
    gameLogger.info(`[auth] 401 Invalid/expired token: ${req.method} ${req.path ?? (req as any).url}`);
